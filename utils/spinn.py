...
def generate_train_data(ncx, ncy, ncz, nx, ny, nz, key):
    
    keys = jax.random.split(key, 3)

    xc = jax.random.uniform(keys[0], (ncx, 1), minval=-1., maxval=1)
    yc = jax.random.uniform(keys[1], (ncy, 1), minval=-1., maxval=1)
    zc = jax.random.uniform(keys[2], (ncz, 1), minval=-1., maxval=1)
